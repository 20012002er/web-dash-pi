...
@plugin_bp.route('/api/plugin/<plugin_id>/data', methods=['GET'])
def get_plugin_data(plugin_id):
    """Fetch structured data from a plugin for the frontend to render.

    This is the KEY new endpoint: the frontend calls it to get the data
    needed to render a plugin's dashboard fragment. It uses the plugin's
    last-saved settings (or empty defaults if none saved), calls
    ``get_data()``, and records the refresh.
    """
    device_config = current_app.config['DEVICE_CONFIG']
    ...
    if refresh_task:
        try:
            refresh_task.record_refresh(plugin_id, "Data Fetch")
        except Exception:
...
def update_now_async():
    ...
    Saves settings to ``plugin_last_settings_<id>`` and queues a
    ``ManualRefresh`` action via the refresh task. Returns 202 Accepted.
    """
    device_config = current_app.config['DEVICE_CONFIG']
    ...
    try:
        ...
        # Handle uploaded files (paths merged into settings)
...
def update_now():
    """Blocking update endpoint — saves settings and runs a synchronous refresh.

    Saves settings to ``plugin_last_settings_<id>``, calls
    ``refresh_task.manual_update(ManualRefresh(...))``, and returns. Image
    generation is no longer performed here — the frontend fetches data via
    ``GET /api/plugin/<plugin_id>/data``.
    """
    device_config = current_app.config['DEVICE_CONFIG']
    refresh_task = current_app.config['REFRESH_TASK']
    ...
    try:
        ...
        # Synchronous manual update — persists the refresh record
        if refresh_task.running:
            refresh_task.manual_update(ManualRefresh(plugin_id, plugin_settings))
        else:
            logger.warning("Refresh task not running; settings saved only.")
...
def add_ticker():
    ...
    Validates the ticker via akshare (used instead of yfinance because
    Yahoo Finance is blocked in mainland China). Wrapped in try/except so
    a missing akshare dependency does not crash the route.
    """
    device_config = current_app.config['DEVICE_CONFIG']
    data = request.get_json()
    ...
    # Validate ticker using akshare. Wrapped in try/except so a missing
    # akshare install (or network failure) doesn't crash the route.
    try:
        ...
        logger.error("akshare not installed; cannot validate ticker '%s'", ticker)
        return jsonify({"error": f"akshare not installed — cannot validate '{ticker}'"}), 500
    except Exception as e:
