...
def display_page():
    """Fullscreen display view — the new frontend shell page."""
...
def skip_to_next():
    ...
    try:
        ...
        # Queue the refresh (non-blocking). New LoopRefresh signature takes
        # (plugin_id, settings=, loop=, plugin_reference=).
        ...
        plugin_name = (
            plugin_config.get("display_name", plugin_ref.plugin_id)
            if plugin_config
            else plugin_ref.plugin_id
        )
...
@main_bp.route('/api/current_state')
def get_current_state():
    """Return the current dashboard state.

    This is the key endpoint the frontend polls every second to know
    which plugin to display.
    """
    refresh_task = current_app.config.get('REFRESH_TASK')
    if not refresh_task:
        return jsonify({"error": "Refresh task not initialized"}), 503
    return jsonify(refresh_task.get_current_state())
...
def get_next_change_time():
    ...
    current_plugin_id = state.get("plugin_id")
    next_plugin_id = state.get("next_plugin_id")
    loop_enabled = state.get("loop_enabled", True)

    # Augment the override dict with a display name when a plugin is pinned
    loop_override = state.get("override")
    if loop_override and loop_override.get("type") == "plugin":
        ...
        if override_plugin:
            ...
            loop_override["display_name"] = override_plugin.get(
                "display_name", loop_override.get("plugin_id")
            )
    ...
    return jsonify({
        ...
        "next_change_in": format_time(int(remaining_seconds)),
